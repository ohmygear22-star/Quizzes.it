...
export function evaluatePreview(product, answers, locale = "en") { const state = replay(product, answers, false); if (state.answers.length !== 5 || state.status !== "payment-required") throw new Error("Complete exactly five preview answers"); return previewInsight(state.scores, locale, sourceQuiz(product)); }
