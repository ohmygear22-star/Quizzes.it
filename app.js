...
async function sendAccessEmail(purchase, token) {
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from,
      to: [purchase.email],
      subject: "Your private quiz link",
      html: "<p>Thank you for your purchase.</p><p><a href=\"" + link + "\">Start your private quiz</a></p><p>This personal link expires in 7 days.</p>"
    })
  });
...
async function markPaid(session) {
  ...
  const token = purchase.pendingToken || crypto.randomBytes(32).toString("base64url");
  purchase.pendingToken = undefined;
